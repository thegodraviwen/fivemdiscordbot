...
if(!args[1]) return message.channel.send(new MessageEmbed().setDescription(`${message.author}, Bir zaman belirtmelisin.`).setTimestamp()).then(x => x.delete({timeout: 5000}));
...
if(kullanici.id === message.author.id)return message.channel.send(new MessageEmbed().setDescription(`${message.author}, Kendini sunucudan cezalıya atılamaz.`).setTimestamp()).then(x => x.delete({timeout: 5000}));
if(kullanici.id === client.user.id)return message.channel.send(new MessageEmbed().setDescription(`${message.author}, Bir botu sunucudan cezalıya atılamaz.`).setTimestamp()).then(x => x.delete({timeout: 5000}));
if(kullanici.id === message.guild.OwnerID) return message.channel.send(new MessageEmbed().setDescription(`${message.author}, Sunucu sahibini sunucudan cezalıya atılamaz.`).setTimestamp()).then(x => x.delete({timeout: 5000}));
...
kullanici.roles.cache.has(manager.Role.JailRolID) ? kullanici.roles.set([manager.Role.BoosterRolID, manager.Role.JailRolID]) : kullanici.roles.set([manager.Role.JailRolID])
...
exports.help = {name: "jail", help: "jail @etiket <süre> <sebep> Belirterek kişiyi jaile gönderebilirsiniz."}
